...
const InfoField: React.FC<{ label: string; children: React.ReactNode }> = ({ label, children }) => (
  <div className="flex flex-col gap-1">
    ...
    <span className="text-sm font-medium text-slate-900">{children || <span className="text-slate-300">—</span>}</span>
  </div>
);
      ...
      {!isEditing && (
        <div className="space-y-8 pb-8 animate-fade-in">
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-6 p-6 bg-white rounded-2xl border border-slate-100">
              ...
              <InfoField label="Alamat">{selectedProject.address || '—'}</InfoField>
          ...
          <section>
            <SectionHeading title="Status & Progres" />
            <SectionCard>
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
                <p className="text-xs font-bold text-slate-500 uppercase tracking-wide shrink-0">Status Acara:</p>
                <div className="relative flex-grow">
                  <select
                    value={selectedProject.status}
                    onChange={e => handleStatusUpdate(e.target.value)}
                    className="w-full appearance-none px-4 py-2.5 text-sm font-semibold rounded-xl border border-slate-200 bg-slate-50 cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                  >
              ...
              <div className="mb-6">
                <div className="flex justify-between text-xs font-semibold text-slate-500 mb-2">
                  ...
                  <span className="text-blue-600">{selectedProject.progress}%</span>
                </div>
                <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full bg-blue-500 transition-all duration-700" style={{ width: `${selectedProject.progress}%` }} />
                </div>
              </div>

              {allSubStatuses.length > 0 && (
                <div className="pt-6 border-t border-slate-100">
                  <p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-4">Tahapan Detail</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {allSubStatuses.map(sub => {
                      const isActive = selectedProject.activeSubStatuses?.includes(sub.name);
                      return (
                        <label key={sub.name} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${isActive ? 'bg-blue-50 border-blue-200' : 'bg-white border-slate-100'}`}>
                          <input type="checkbox" className="w-4 h-4 rounded text-blue-600" checked={!!isActive} onChange={e => handleSubStatusToggle(sub.name, e.target.checked)} />
                          <span className={`text-sm font-medium ${isActive ? 'text-blue-900' : 'text-slate-600'}`}>{sub.name}</span>
                        </label>
                      );
                    })}
                  </div>
          ...
          <section>
            ...
            <SectionCard>
              <div className="grid grid-cols-2 gap-8">
                ...
                <InfoField label="Add-ons">{selectedProject.addOns?.map(a => a.name).filter(Boolean).join(', ') || '—'}</InfoField>
              </div>
            </SectionCard>
          </section>
